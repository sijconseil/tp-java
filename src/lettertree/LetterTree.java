...
public class LetterTree {
	...
	private boolean containsRecursive(String word, int currentIndex, Node currentNode) {
		if(currentIndex==word.length()) return currentNode.terminal;
	...
	private int maxDepth(Node currentNode) { // retourne la profondeur maximum de l'arbre
		...
		// petit rappel :il est parfois plus facile de commencer par la condition d'arret:
		...
		if(currentNode.children.isEmpty())return 1;
		int max = -1;
		for(Node node : currentNode.children.values()) {
			int tmpDepth = maxDepth(node);
			if(tmpDepth>max) max = tmpDepth;
		}
		return max+1;
	}
	...
	private int countWords(Node currentNode) {
		// la seule chose qui change est la condition d'etre terminal pour ajouter 1 au total
