...
public class LetterTree {
	...
	private boolean containsDoubleChar(Node current) {
		// première condition d'arret : si je n'ai plus de fils, je suis en bout de branche et je return false;
...
//		}
		...
		// seconde condition d'arret : si pour une de mes lettre, j'ai un fils qui contient la meme dans sa "map" children, 
		...
		for(Character letter: current.children.keySet()) {
			...
			if(child.children.containsKey(letter)) return true;
		}
		
		// dans le cas general ou je ne m'arrete pas, je lance la récursion sur mon fils. Si un de mes fils trouve, alors j'arrete
		// la récursion et je n'itere pas sur les suivants
		for(Node child: current.children.values()) {
			if(containsDoubleChar(child)) return true;
		}
		// la fin de la boucle, aucun de mes fils n'a donc retourné vrai et n'a trouvé la double lettre, mes conditions 
		// d'arret montrent aussi qu'elle n'est pas sur le noeud courant, donc je retourne false
		return false;
	}
...
//		}
	...
	public void load(String filePath) throws Exception { // charge le fichier dont le chemin est donné en paramètre
		...
		try {
			...
			while (s!=null) {
				if(Character.isLowerCase(s.charAt(0)))this.add(s.trim());
				s = reader.readLine();
			}
